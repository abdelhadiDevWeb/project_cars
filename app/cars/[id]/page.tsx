...
export default function CarDetailsPage() {
  ...
  useEffect(() => {
    const fetchCar = async () => {
      try {
        ...
        if (!res.ok) {
          setError(data?.message || t("Voiture non trouvée"));
          setLoading(false);
          return;
        }
  ...
  const handleUpdate = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      if (!token) {
        setError(t("Vous devez être connecté pour modifier cette voiture"));
        setIsUpdating(false);
        return;
      }
  ...
  const handleDelete = async () => {
    ...
    if (!confirm(t("Êtes-vous sûr de vouloir supprimer cette voiture ? Cette action est irréversible."))) {
      return;
    }
    ...
    try {
      const token = localStorage.getItem('token');
      if (!token) {
        setError(t("Vous devez être connecté pour supprimer cette voiture"));
        setIsDeleting(false);
        return;
      }
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {isOwner && !editing && (
              <div className="bg-teal-50 border border-teal-200 rounded-lg p-4 mb-6">
                <div className="flex items-center justify-between">
                  <p className="text-teal-800 font-semibold">{t("Vous êtes le propriétaire de cette voiture")}</p>
            ...
            {isOwner && editing && (
              <div className="bg-white rounded-2xl shadow-md p-6 border border-gray-200 mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-4">{t("Modifier la voiture")}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t('Marque')} *</label>
                    <input
                      type="text"
                      name="brand"
                      required
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t('Modèle')} *</label>
                    <input
                      type="text"
                      name="model"
                      required
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t('Année')} *</label>
                    <input
                      type="number"
                      name="year"
                      required
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t('Kilométrage (km)')} *</label>
                    <input
                      type="number"
                      name="km"
                      required
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t('Prix (DA) *')}</label>
                    <input
                      type="number"
                      name="price"
                      required
                ...
                <div className="flex gap-4 mt-6">
                  ...
                  >
                    {isUpdating ? t('Mise à jour...') : t('Enregistrer')}
                  </button>
                  <button
                    onClick={() => {
                      setEditing(false);
            ...
            <div className="bg-white/95 backdrop-blur-md rounded-3xl shadow-2xl p-8 border-2 border-gray-200/50">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="p-5 bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl border-2 border-blue-200 hover:shadow-lg transition-all">
                  <p className="text-xs text-gray-600 mb-2 font-medium">{t('Année')}</p>
                ...
                <div className="p-5 bg-gradient-to-br from-teal-50 to-cyan-50 rounded-2xl border-2 border-teal-200 hover:shadow-lg transition-all">
                  <p className="text-xs text-gray-600 mb-2 font-medium">{t('Kilométrage')}</p>
                ...
                <div className="p-5 bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl border-2 border-green-200 hover:shadow-lg transition-all">
                  <p className="text-xs text-gray-600 mb-2 font-medium">{t('Statut')}</p>
                  <p className="text-xl font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
                    {car.status === 'no_proccess' ? t('Non traité') :
                     car.status === 'en_attente' ? t('En attente') :
                     car.status === 'actif' ? t('Actif') :
                     car.status === 'sold' ? t('Vendu') : car.status}
                  </p>
                </div>
                <div className="p-5 bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl border-2 border-purple-200 hover:shadow-lg transition-all">
                  <p className="text-xs text-gray-600 mb-2 font-medium">{t('Prix')}</p>
                  <p className="text-2xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">{car.price.toLocaleString()} {t('DA')}</p>
                </div>
              </div>
              ...
              {car.vin && (
                <div className="mt-4 w-full">
                  <p className="text-sm font-semibold text-teal-800 mb-2">{t('Numéro VIN')}</p>
              ...
              {(car.color || car.ports || car.boite || car.type_gaz || car.type_enegine || car.description || car.accident !== undefined || car.usedby) && (
                <div className="mt-6 bg-gradient-to-br from-gray-50 to-gray-100 rounded-3xl shadow-xl p-8 border-2 border-gray-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <p className="text-xs text-gray-500 mb-1 font-medium">{t('Couleur')}</p>
                        <p className="text-base font-semibold text-gray-900">{car.color}</p>
                      </div>
                    )}
                    {car.ports && (
                      <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                        <p className="text-xs text-gray-500 mb-1 font-medium">{t('Nombre de portes')}</p>
                        <p className="text-base font-semibold text-gray-900">{car.ports}</p>
                      </div>
                    )}
                    {car.boite && (
                      <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                        <p className="text-xs text-gray-500 mb-1 font-medium">{t('Boîte de vitesses')}</p>
                    ...
                    {car.type_gaz && (
                      <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                        <p className="text-xs text-gray-500 mb-1 font-medium">{t('Type de carburant')}</p>
                        <p className="text-base font-semibold text-gray-900">
                    ...
                    {car.type_enegine && (
                      <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                        <p className="text-xs text-gray-500 mb-1 font-medium">{t('Type de moteur')}</p>
                        <p className="text-base font-semibold text-gray-900">{car.type_enegine}</p>
                      </div>
                    )}
                    {car.usedby && (
                      <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                        <p className="text-xs text-gray-500 mb-1 font-medium">{t('Utilisé par')}</p>
                        <p className="text-base font-semibold text-gray-900">{car.usedby}</p>
                      </div>
                    )}
                    {car.accident !== undefined && (
                      <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                        <p className="text-xs text-gray-500 mb-1 font-medium">{t('Accident')}</p>
                        <p className="text-base font-semibold">
                          {car.accident ? (
                            <span className="text-red-600">{t('Oui')}</span>
                          ) : (
                            <span className="text-green-600">{t('Non')}</span>
                          )}
                        </p>
                      </div>
                    )}
                    {car.description && (
                      <div className="md:col-span-2 lg:col-span-3 bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                        <p className="text-xs text-gray-500 mb-2 font-medium">{t('Description')}</p>
                ...
                <div className="bg-gradient-to-br from-teal-50 via-cyan-50 to-blue-50 rounded-3xl shadow-2xl p-8 border-2 border-teal-200/50 mt-6">
                  ...
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-sm text-gray-600 text-center max-w-md">
                      {t('Scannez ce code QR pour vérifier le statut de vérification de ce véhicule')}
                    </p>
                  </div>
                </div>
              )}
                    ...
                    <div key={idx} className="mb-8 last:mb-0 p-6 bg-white/80 backdrop-blur-sm rounded-2xl border-2 border-purple-200/50">
                      {appointment.id_workshop && (
                        <div className="mb-4 p-4 bg-gradient-to-r from-purple-100 to-indigo-100 rounded-xl border border-purple-200">
                          <p className="text-xs text-gray-600 mb-1 font-medium">{t('Atelier de vérification')}</p>
                      ...
                      {appointment.images && appointment.images.length > 0 && (
                        <div className="mb-6">
                          <p className="text-base font-bold text-gray-800 mb-4 flex items-center gap-2">
                            ...
                            {t('Images de vérification')} ({appointment.images.length})
                          </p>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white/95 backdrop-blur-md rounded-3xl shadow-2xl p-6 border-2 border-gray-200/50 sticky top-24 space-y-6">
              <div className="p-6 bg-gradient-to-br from-teal-50 to-cyan-50 rounded-2xl border-2 border-teal-200">
                <p className="text-sm text-gray-600 mb-2 font-medium">{t('Prix')}</p>
              ...
              <div className="pt-6 border-t border-gray-200">
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600">{t('Vendeur vérifié')}</p>
                        </div>
                    </div>
                  </div>
                </div>
