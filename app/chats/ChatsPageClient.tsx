...
export default function ChatsPageClient() {
    ...
    try {
      ...
      if (data.ok && data.chats) {
        ...
        const validChats = data.chats.filter(
          (chat: Chat) =>
            chat && chat.otherUser && chat.otherUser.id && user && user._id !== chat.otherUser.id,
        );
        setChats(validChats);
        // Calculate total unread count
        const total = validChats.reduce(
  ...
  useEffect(() => {
    ...
    if (userIdParam && chats.length > 0 && user && !showChatModal) {
      ...
      const targetChat = chats.find(
        (chat) => chat.otherUser.id === userIdParam && user._id !== chat.otherUser.id,
      );
  ...
  const handleChatClick = async (chat: Chat) => {
    ...
    if (user && user._id === chat.otherUser.id) return;
    // If clicking the same chat that's already open, do nothing
    if (selectedChat && selectedChat.id === chat.id && showChatModal) return;
    ...
    // Refresh chats after opening to update unread count
    setTimeout(() => {
      fetchChats();
    }, 800);
  };
  ...
  const formatTime = (dateString: string) => {
    ...
    if (minutes < 1)
      return language === 'en' ? 'Just now' : language === 'ar' ? 'الآن' : "À l'instant";
    if (minutes < 60) {
      return language === 'en'
        ? `${minutes} min ago`
        : language === 'ar'
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          <svg
            className="animate-spin h-12 w-12 text-teal-500 mx-auto"
                      ...
                      <div className="relative flex-shrink-0">
                        {chat.otherUser.profileImage ? (
                          <div className="relative w-16 h-16 lg:w-20 lg:h-20 rounded-2xl overflow-hidden ring-2 ring-white shadow-lg group-hover:ring-teal-300 transition-all duration-200 group-hover:scale-105">
                            <Image
                              src={getImageUrl(chat.otherUser.profileImage) || '/images/default-avatar.png'}
                        ...
                        <div className="mt-1">
                          {chat.lastMessage ? (
                            ...
                            >
                              <span
                                className={
                                  chat.lastMessage.id_sender.id === user?._id ? 'text-teal-600 font-medium' : ''
                                }
                              >
